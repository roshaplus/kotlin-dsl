...
class KotlinScriptSource(val source: ScriptSource) {
    ...
    val script by lazy { convertLineSeparators(scriptResource.text!!, "\n") }
...
inline fun <T> LoadedScriptClass<T>.withLocationAwareScriptEvaluationExceptionHandling(
    ...
    try {
        ...
        throw maybeLocateException(scriptSource, e)
    }


private
fun LoadedScriptClass<*>.maybeLocateException(scriptSource: KotlinScriptSource, e: Throwable): Throwable {
    ...
    tailrec fun inferLocationFrom(exception: Throwable): Throwable {
        ...
        exception.stackTrace.find { matches(it) }
            ?.run { return LocationAwareException(actualException, scriptSource.source, lineNumber.takeIf { it >= 0 }) }
